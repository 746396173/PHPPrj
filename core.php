...
// 	curl_setopt($ch, CURLOPT_HEADER, TRUE);
	...
	log_message("debug","[CH]".$ch);
	$data = curl_exec($ch);
	
	if (preg_match("/ptui_checkVC\('(.*)','(.*)','(.*)','(.*)'\);/", $data, $verify))
	{
		//ptui_checkVC('0','!KIF','\x00\x00\x00\x00\x02\xc7\x9a\x67','70a910f3a82ff5db42ba5e53368d404a4d6f07553b39430687c667c67ebe25fd84c4317011792c414652f1dd29b6ec92');
		log_message("debug","【data】".$data);
		log_message("debug","【verify】".$verify);
		logarray($verify);
		log_message("debug",$v);
...
function login($uid, $passwd, $verify)
{
	...
	//$url = "http://ptlogin2.qq.com/login?u={$uid}&p={$passwd}&verifycode={$verify}&remember_uin=1&login2qq=1&aid=1003903&u1=http%3A%2F%2Fweb.qq.com%2Floginproxy.html%3Flogin2qq%3D1%26webqq_type%3D10&h=1&ptredirect=0&ptlang=2052&from_ui=1&pttype=1&dumy=&fp=loginerroralert&action=8-38-447467&mibao_css=m_webqq&t=3&g=1";
